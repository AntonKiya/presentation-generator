...
src/presentation-layout/
  presentation-layout-engine.service.ts
  presentation-layout.types.ts
  presentation-layout.constants.ts
  internal/                 Внутренняя раскладка составных elements

src/presentation-theme/
  presentation-default-theme.ts

src/presentation-export/
  pptx/                    PPTX contract, preflight, renderer, writer adapter
```
...
`presentation-layout/` - общий deterministic layout слой для renderer-ов. Он не
меняет DSL и не занимается генерацией контента. Его задача - превратить дерево
containers/elements в стабильные boxes, которые затем используют HTML preview и
PPTX export.
...
`columns` может быть числом или `"auto"`. Значение `"auto"` сейчас разрешается
общим layout engine в стабильное количество колонок по числу children.
...
Для текущего визуального MVP renderer-ы используют локальные assets из корня
проекта:

- `images.jpeg` - временное default image asset для всех `image` elements;
- `icon.png` - временная default icon для визуальных card-паттернов.

Если `images.jpeg` есть, HTML preview встраивает его как data URL, а PPTX export
вставляет его как native image с `cover` sizing. Если файла нет, renderer-ы
возвращаются к мягкому текстовому placeholder. Если `icon.png` есть, карточки
используют его внутри мягкой icon-плашки; если файла нет, используется
текстовый fallback-символ.
...
HTML preview использует общий `PresentationLayoutEngineService`, а не отдельную
CSS-интерпретацию container tree. Сначала строится layout plan с рассчитанными
boxes, затем HTML renderer проходит DSL tree и layout tree параллельно и
отрисовывает элементы в абсолютных координатах внутри wide 16:9 slide canvas.

Это нужно, чтобы web-preview и PPTX export были максимально близки по:

- safe area;
- размерам и позициям контейнеров;
- поведению `stack`, `row`, `grid`;
- разрешению `grid.columns = "auto"`;
- внутренней раскладке `cards`, включая icon/title/body/metric zones;
- внутренней раскладке составных elements, включая arrow/takeaway-вариант
  `bullets`;
- базовой типографике, цветам и spacing из `PRESENTATION_DEFAULT_THEME`.

Полной pixel-perfect идентичности между браузером и PowerPoint гарантировать
нельзя: у них разные text engines, переносы строк, таблицы и charts. Но текущий
фундамент должен давать одинаковую композицию, сетку, отступы и базовое
визуальное поведение.

Есть renderer smoke contract test: валидная презентация со всеми MVP elements
должна отрендериться без exception, содержать ожидаемые HTML-блоки и использовать
layouted absolute boxes.
...
valid presentation JSON
  ...
  -> shared presentation layout engine
...
При этом оба renderer-а должны опираться на один shared layout plan, чтобы
пользователь видел в web-preview максимально близкую композицию к скачанному
`.pptx`.
...
- bullets остаются editable text, а визуальные markers рендерятся как
  arrow/takeaway markers;
...
- images используют временный default image asset, если он доступен, иначе
  fallback placeholder.

Цель MVP - стабильный, детерминированный и редактируемый `.pptx`, который
максимально близок к web-preview по layout semantics. Pixel-perfect parity не
гарантируется из-за различий браузера и PowerPoint, но расхождения не должны
быть следствием двух независимых layout engines.
...
bullets             -> arrow/takeaway markers + editable text boxes
image               -> native image from temporary default asset, fallback placeholder
cards               -> soft panels + optional icon + title/body/metric text
table               -> native table
...
Layout engine является отдельным общим слоем для renderer-ов. Он получает DSL
tree и возвращает layouted model:
...
type LayoutedNode = {
  ...
  path: string;
  box: Box;
  internal?: ElementInternalLayout;
  children?: LayoutedNode[];
};
```

Единицы layout engine сейчас совпадают с PowerPoint inches, потому что PPTX
writer использует эту систему координат. HTML preview переводит эти units в
проценты/preview pixels внутри `1120 x 630` canvas. `gap` и `padding` из DSL
сейчас задаются в preview-пикселях, поэтому layout engine детерминированно
переводит их в layout units относительно wide 16:9 canvas.
...
HTML renderer следует тому же правилу: он не делает собственный flex/grid layout
для DSL, а отрисовывает элементы по заранее рассчитанным boxes.

У сложных elements может быть дополнительная внутренняя раскладка в
`src/presentation-layout/internal/`. Это не новая DSL-сущность и не данные,
которые должна генерировать LLM. Это renderer-facing geometry, которую backend
строит из уже валидного DSL.

Сейчас такой слой есть для `bullets` и `cards`.

Для `bullets` layout engine считает box каждого пункта, box arrow marker и box
editable text. HTML preview и PPTX export используют одни и те же внутренние
координаты, поэтому перенос строк внутри одного takeaway-пункта не должен
наезжать на следующий пункт. Если пунктов слишком много для доступной области,
layout возвращает warning `BULLETS_MAY_OVERFLOW` и вписывает список в доступный
box без изменения DSL. Arrow marker привязывается к первой строке пункта, а не
к вертикальному центру всего многострочного блока.

Для `cards` layout engine считает сетку карточек, `cardBox`, `iconBox`,
`titleBox` и `bodyBox`. Если title карточки выглядит как числовой показатель
(`70%`, `+10%`, `2050`), карточка получает variant `metric`: крупный numeric
title без icon-плашки. Renderer-ы не должны заново решать внутреннюю геометрию
карточек: они только рисуют shape/icon/editable text по уже рассчитанным boxes.
Размеры шрифтов внутри карточек также считаются этим internal layout слоем:
если контента слишком много для доступной карточки, backend уменьшает card
typography в рамках renderer layout, не меняя DSL и не требуя repair.
...
Точные размеры `wide-16-9` живут в `PRESENTATION_LAYOUT_SLIDE_SIZES`. PPTX
contract экспортирует их как `PPTX_EXPORT_SLIDE_SIZES`, чтобы writer, layout
engine и export contract не расходились. Safe area и эвристики первого layout
engine живут в `presentation-layout.constants.ts`, а не в базовом DSL.

Текущий layout engine:

- строит `PresentationLayout` и `PresentationSlideLayout`;
...
- экспортирует общий `resolveCardsColumns`, чтобы HTML и PPTX одинаково
  определяли сетку cards;
- строит внутренний layout для `bullets` и `cards`, чтобы составные элементы не
  рассчитывались отдельно в HTML и PPTX renderer-ах;
- не мутирует `presentation JSON`;
- не вызывает writer и не знает про `pptxgenjs`;
- возвращает layout warnings вроде `BOX_TOO_SMALL` и `BULLETS_MAY_OVERFLOW`,
  если рассчитанная область слишком мала для стабильного render.

Эвристики высоты нужны renderer-ам для первого стабильного layout pass. Они не
являются новой семантикой DSL и не заменяют template validation.

PPTX-facing файлы в `src/presentation-export/pptx/layout/` остаются как thin
compatibility layer: старые имена `PptxLayoutEngineService`,
`PptxPresentationLayout`, `resolveGridColumns` продолжают экспортироваться, но
реальная логика находится в `src/presentation-layout/`.

### Renderer contract

PPTX renderer получает валидный `presentation JSON` и заранее рассчитанный
`PptxPresentationLayout` / shared `PresentationLayout`. Он не считает layout
повторно и не импортирует `pptxgenjs` напрямую.
...
- рендерит `bullets` как arrow/takeaway markers + editable text boxes по shared
  internal bullets layout;
- добавляет тонкую accent line для `slot: "title"`;
- рендерит `image` как native image из временного default asset без декоративной
  frame-рамки, либо fallback placeholder shape + text;
- рендерит `cards` как soft panels, optional default icon и editable text boxes
  по shared internal cards layout;
...
endpoint для скачивания `.pptx` реализован как
...
Текущий базовый theme layer находится здесь:
...
DSL element type + optional abstract style/source fields
  -> PresentationTheme
  -> HTML CSS / concrete PPTX writer options
```

Сейчас есть только один минимальный `default` theme:
...
- arrow/takeaway markers;
- card icon и metric-card tokens;
- title accent visual tokens.

Текущая default theme использует modern editorial palette: теплый светлый фон,
глубокий синий accent/text, мягкие card panels без жесткой UI-рамки и крупную
типографику для заголовков, карточек и numeric metrics. Классические bullets
визуально превращаются в arrow/takeaway list, а `cards` могут выглядеть как
soft icon cards или metric cards без изменения базового DSL. Размеры шрифтов
остаются частью общего visual contract и не должны произвольно меняться между
будущими темами; новые темы должны менять в первую очередь цвета, font family,
accent/surface/border/chart palette и декоративные визуальные токены.

Это не финальная дизайн-система и не набор пользовательских тем. На текущем
этапе theme layer нужен только как аккуратная граница, чтобы HTML и PPTX renderer
не хранили независимые стили прямо внутри себя и чтобы будущие осмысленные темы
можно было добавить без изменения DSL.
...
- guaranteed pixel-perfect parity between browser and PowerPoint;
...
shared Layout engine, shared Theme, PPTX writer adapter contract, PPTX Preflight,
PPTX Renderer и полный PPTX pipeline.
...
- renderer smoke с shared layout boxes;
- PPTX writer adapter;
- PPTX export Preflight;
- shared Layout engine через PPTX compatibility API;
- PPTX Renderer;
- shared/PPTX Theme;
...
- HTML preview и PPTX export используют общий layout/theme фундамент, но
  guaranteed pixel-perfect parity между browser и PowerPoint пока не обещается;
