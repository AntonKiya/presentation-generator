...
Для текущего визуального MVP renderer-ы используют локальный `images.jpeg` из
корня проекта как временное default image asset. Если файл есть, HTML preview
встраивает его как data URL, а PPTX export вставляет его как native image с
`cover` sizing. Если файла нет, renderer-ы возвращаются к мягкому текстовому
placeholder.
...
- внутренней раскладке `cards`;
- внутренней раскладке составных elements, сейчас прежде всего `bullets`;
...
- bullets остаются editable text, а декоративные markers рендерятся отдельными
  accent shapes;
...
bullets             -> accent marker shapes + editable text boxes
...
Для `bullets` layout engine считает box каждого пункта, box декоративного marker
и box editable text. HTML preview и PPTX export используют одни и те же
внутренние координаты, поэтому перенос строк внутри одного bullet не должен
наезжать на следующий bullet. Если пунктов слишком много для доступной области,
layout возвращает warning `BULLETS_MAY_OVERFLOW` и вписывает список в доступный
box без изменения DSL.

Для `cards` layout engine считает сетку карточек, `cardBox`, `titleBox` и
`bodyBox`. Renderer-ы не должны заново решать внутреннюю геометрию карточек:
они только рисуют shape и editable text по уже рассчитанным boxes.
...
- рендерит `bullets` как accent marker shapes + editable text boxes по shared
  internal bullets layout;
- добавляет тонкую accent line для `slot: "title"`;
- рендерит `image` как native image из временного default asset, либо fallback
  placeholder shape + text;
- рендерит `cards` как editable shapes + text boxes по shared internal cards
  layout;
...
- bullet marker и title accent visual tokens.

Текущая default theme использует теплую heritage palette для базового
презентационного вида. Card typography намеренно усилена относительно первого
чернового варианта, чтобы карточки выглядели менее пустыми без добавления
декоративных accent-элементов. Размеры шрифтов остаются частью общего visual
contract и не должны произвольно меняться между будущими темами; новые темы
должны менять в первую очередь цвета, font family, accent/surface/border/chart
palette и декоративные визуальные токены.
